...
namespace pjpl {
    ...
    namespace wstr {
        ...
        std::wstring::size_type after(const std::wstring &subject, const std::wstring &token, std::wstring::size_type start) {
            if (start < subject.length()) {
                std::wstring::size_type pos = subject.find(token, start);
        ...
        std::wstring adjustToken(const std::wstring &str, size_t startPos, const std::vector<std::wstring> &tokens) {
            for (auto &it : tokens ) {
        ...
        std::wstring adjustNearestToken(const  std::wstring &str, size_t &start, size_t &stop, const std::vector<std::wstring> &tokens) {
        ...
        std::wstring between(
                ...
                , const std::wstring &leftLim
                , const std::wstring &rightLim
                , std::size_t &start
                , std::size_t &cursor
                , bool trim)
        {
        ...
        bool empty(const std::wstring &buf)
        {
            if (buf.size() == 0) {
                return true;
            } else {
        ...
        std::wstring token(const std::wstring &str, size_t &start, size_t &stop, wchar_t between) {
            size_t sz = str.size();
            size_t stop_;
            ...
            if (between != '\0') {
                ...
                std::wstring ret = str.substr(start , stop_ - start);
                stop = ++stop_;
        ...
        std::wstring token(const std::wstring &s, size_t &start, size_t &stop)
        {
            if (start >= s.size()) {
                return L"";
            }

            std::wstring operat = pjpl::wstr::adjustToken(s, start, operators);
            if (operat.length() > 0) {
            ...
            ix = start;
            if (pjpl::ispunct(s[ix]) && s[ix] != '_' && s[ix] != '.') {
                /*
            ...
            if (isdigit(s[ix])) {
                ...
                if ((s[ix] == 'e' || s[ix] == 'E')&&(s[ix + 1] == '+' || s[ix + 1] == '-') && isdigit(s[ix + 2])) {//wyklucza 123e+123
                    ...
                    if (std::isalpha(s[ix]) || s[ix] == '_') {
                        ...
                        if (ix >= stop || isspace(s[ix]) || pjpl::ispunct(s[ix])) {
                            stop = ix;
                            return s.substr(start, stop - start);
                        }
                    }
        ...
        std::wstring trimLeft(const std::wstring &buf)
        {
            if (buf.size() == 0)
                return L"";
            size_t i = 0;
        ...
        std::wstring trimRight(const std::wstring &buf)
        {
            if (buf.size() == 0)
                return L"";
            size_t i = buf.size() - 1;
        ...
        std::wstring read(const std::wstring &source, size_t &start, size_t &stop, const std::wstring &lim)
        {
            if (source.size() == 0) {
                return L"";
            }
            std::wstring::size_type source_length = source.length();
            if(start >= source_length) {
                stop = source_length;
                return L"";
            }

            std::wstring::size_type end = source.find(lim, start);
            std::wstring out;
        ...
        std::vector<std::wstring> split( const std::wstring& str, const std::wstring& rozdzielacz, bool trim) {
            ...
            for ( iter pos; ( pos = std::search( i , str.end(), rozdzielacz.begin(), rozdzielacz.end() ) ) != str.end(); ) {
                token = std::wstring( i, pos );
                if (rozdzielacz.find(token) == std::wstring::npos) {
                    if (trim){
